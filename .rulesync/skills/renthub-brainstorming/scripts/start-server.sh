...
#
# 在随机高端口启动服务器，输出含 URL 的 JSON。
# 每个会话使用独立目录，避免冲突。
#
# 选项:
#   --project-dir <path>  将会话文件存到 <path>/.superpowers/brainstorm/
#                         而非 /tmp。服务器停止后文件仍保留。
#   --host <bind-host>    绑定主机/网卡（默认: 127.0.0.1）。
#                         远程/容器环境可使用 0.0.0.0。
#   --url-host <host>     返回的 URL JSON 中显示的主机名。
#   --foreground          在前台当前终端运行（不后台化）。
...
# 部分环境会回收脱离/后台进程，检测到则自动前台运行。
if [[ -n "${CODEX_CI:-}" && "$FOREGROUND" != "true" && "$FORCE_BACKGROUND" != "true" ]]; then
  FOREGROUND="true"
fi

# Windows/Git Bash 会回收 nohup 后台进程，检测到则自动前台运行。
...
# 解析 harness 的 PID（本脚本的祖父进程）。
# $PPID 是 harness 为运行本脚本创建的临时 shell — 本脚本退出时它会结束。
...
# 启动服务器，输出写入日志
# 使用 nohup 在 shell 退出后仍存活；disown 从作业表移除
nohup env BRAINSTORM_DIR="$SESSION_DIR" BRAINSTORM_HOST="$BIND_HOST" BRAINSTORM_URL_HOST="$URL_HOST" BRAINSTORM_OWNER_PID="$OWNER_PID" node server.cjs > "$LOG_FILE" 2>&1 &
SERVER_PID=$!
disown "$SERVER_PID" 2>/dev/null
echo "$SERVER_PID" > "$PID_FILE"

# 等待 server-started 消息（查日志）
for i in {1..50}; do
  if grep -q "server-started" "$LOG_FILE" 2>/dev/null; then
    # 短窗口内确认进程仍存活（捕获被回收的情况）
    alive="true"
    for _ in {1..20}; do
